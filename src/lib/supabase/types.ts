...
//     USING: check_is_admin_master()
//   Policy "Analistas can view school users" (SELECT, PERMISSIVE) roles={public}
//     USING: (auth.uid() IN ( SELECT usuarios_escola_1.id    FROM usuarios_escola usuarios_escola_1   WHERE ((usuarios_escola_1.escola_id = usuarios_escola_1.escola_id) AND (usuarios_escola_1.perfil = 'analista_de_compliance'::text) AND (usuarios_escola_1.ativo = true))))
//   Policy "Compliance Global View Users" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: is_compliance_member()
//   Policy "Director Compliance Read All Users" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: (EXISTS ( SELECT 1    FROM usuarios_escola usuarios_escola_1   WHERE ((usuarios_escola_1.id = auth.uid()) AND (usuarios_escola_1.perfil = 'DIRETOR_COMPLIANCE'::text))))
//   Policy "Director View All Users" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: is_compliance_director()
//   Policy "Directors can view all users" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: (EXISTS ( SELECT 1    FROM usuarios_escola usuarios_escola_1   WHERE ((usuarios_escola_1.id = auth.uid()) AND (usuarios_escola_1.perfil = 'DIRETOR_COMPLIANCE'::text))))
...
//     USING: ((escola_id IS NOT NULL) AND (escola_id = get_user_escola_id(auth.uid())))
...
//   Policy "usuarios_escola_select_school_or_admin" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: ((id = ( SELECT auth.uid() AS uid)) OR ((escola_id IS NOT NULL) AND (escola_id = (((auth.jwt() -> 'app_metadata'::text) ->> 'escola_id'::text))::uuid)) OR ((auth.jwt() ->> 'role'::text) = 'admin'::text))
