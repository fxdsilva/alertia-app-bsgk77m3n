...
export default function Support() {
  ...
  const ChannelCard = ({ channel }: { channel: OfficialChannel }) => (
    <Card className="flex flex-col h-full hover:shadow-md transition-shadow border-slate-200">
      ...
      <CardFooter className="pt-0">
        <Button
          variant="outline"
          className="w-full gap-2 text-emerald-700 hover:text-emerald-800 hover:bg-emerald-50 border-emerald-200"
          onClick={() =>
  ...
  const hasChannels =
    data &&
    ((data.brasil && data.brasil.length > 0) ||
      (data.mato_grosso && data.mato_grosso.length > 0))
  const hasEmergency = data && data.emergency && data.emergency.length > 0
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 font-sans text-slate-900 animate-fade-in">
          ...
          <div className="space-y-10">
            ...
            {hasChannels && (
              <Tabs defaultValue="br" className="w-full">
                <TabsList className="grid w-full grid-cols-2 mb-8 bg-slate-100 p-1 h-auto rounded-lg">
                  <TabsTrigger
                    value="br"
                    className="text-base py-3 data-[state=active]:bg-white data-[state=active]:shadow-sm transition-all rounded-md"
                  >
                ...
                >
                  {data.mato_grosso && data.mato_grosso.length > 0 ? (
                    ...
                    <div className="text-center py-10 text-slate-500 bg-white rounded-xl border border-slate-200">
                      Canais de Mato Grosso estão sendo atualizados no momento.
                    </div>
                  )}
                </TabsContent>
                ...
                >
                  {data.brasil && data.brasil.length > 0 ? (
                    ...
                    <div className="text-center py-10 text-slate-500 bg-white rounded-xl border border-slate-200">
                      Canais do Brasil estão sendo atualizados no momento.
                    </div>
                  )}
                </TabsContent>
              </Tabs>
      ...
      <footer className="py-8 text-center text-sm text-slate-500 border-t border-slate-200 mt-auto bg-white">
        <p>
