...
export default function CodeOfConduct() {
  ...
  useEffect(() => {
    ...
    const fetchDoc = async () => {
      try {
        const doc = await portalService.getCodeOfConduct(selectedSchool.id)
        setDocument(doc)
      } catch (error) {
        console.error(error)
        toast.error('Erro ao carregar o Código de Conduta.')
      } finally {
        setLoading(false)
      }
    }
  ...
  return (
    <div className="container mx-auto max-w-4xl space-y-6 animate-fade-in py-6">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-3xl font-bold text-primary">Código de Conduta</h1>
      </div>

      <Card className="min-h-[60vh]">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <FileText className="h-5 w-5" /> Documento Oficial
          </CardTitle>
        </CardHeader>
          ...
          {loading ? (
            ...
            <div className="text-center text-muted-foreground">
              <p className="text-lg mb-4">
                O Código de Conduta ainda não foi disponibilizado por esta
                instituição.
              </p>
              <p className="text-sm">
