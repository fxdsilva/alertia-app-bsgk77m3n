...
export default function CodeOfConductManager() {
  ...
  const fetchDoc = async () => {
    ...
    try {
      const doc = await adminService.getCodeOfConduct(selectedSchool.id)
      setDocument(doc)
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      await adminService.upsertCodeOfConduct(
        selectedSchool.id,
        file,
        description,
      )
  ...
  const handleUpdate = async () => {
    ...
    setLoading(true)
    try {
      await adminService.updateCodeOfConductDescription(
        document.id,
        description,
      )
  ...
  const handleDelete = async () => {
    ...
    setLoading(true)
    try {
      await adminService.deleteCodeOfConduct(document.id)
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold">Gestão: Código de Conduta</h1>
        {!canEdit && (
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2 p-4 border rounded bg-muted/20">
                  ...
                  <div className="flex-1 overflow-hidden">
                    <p className="truncate font-medium">Documento Vigente</p>
                    <a
              ...
              {!document && (
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    value={description}
                    onChange={(e) => setDescription(e.target.value)}
                    placeholder="Ex: Código de conduta aprovado em assembleia..."
                    disabled={!canEdit}
                  />
                </div>
              )}
