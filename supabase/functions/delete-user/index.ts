  ...
  try {
    ...
    if (error) {
      if (error.message?.includes('Database error deleting user')) {
        return new Response(
          JSON.stringify({ error: 'Não é possível excluir o usuário, pois existem registros associados a ele no sistema (denúncias, auditorias, etc.). Recomendamos inativar o usuário.' }),
          {
